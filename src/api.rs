...
async fn get_transactions(
    ...
    match get_paginated_transactions(&state, page, limit).await {
        ...
        Err(e) => {
            ...
            Json(ApiResponse::<Vec<crate::state::Transaction>>::error(e.to_string()))
        }
    }
}
...
async fn get_transaction(
    ...
    match state.node.chain_state.get_transaction(&hash).await {
        ...
        Ok(None) => Json(ApiResponse::<crate::state::Transaction>::error("Transaction not found".to_string())),
        Err(e) => {
            tracing::error!("Failed to get transaction {}: {}", hash, e);
            Json(ApiResponse::<crate::state::Transaction>::error(e.to_string()))
        }
    }
}
...
async fn submit_transaction(
    ...
    if tx_req.amount == 0 {
        return Json(ApiResponse::<Transaction>::error("Amount cannot be zero".to_string()));
    }

    if tx_req.gas_price == 0 {
        return Json(ApiResponse::<Transaction>::error("Gas price cannot be zero".to_string()));
    }
    ...
    match state.node.chain_state.validate_transaction(&convert_consensus_tx_to_state(&tx)).await {
        Ok(true) => {
            // Add to pending transactions
            let state_tx = convert_consensus_tx_to_state(&tx);
            match state.node.chain_state.add_pending_transaction(state_tx).await {
                Ok(_) => {
        ...
        Ok(false) => {
            Json(ApiResponse::<Transaction>::error("Transaction validation failed".to_string()))
        },
        Err(e) => {
...
async fn add_validator(
    ...
    if validator_req.stake < 1000 {
        return Json(ApiResponse::<Validator>::error("Minimum stake is 1000".to_string()));
    }

    if validator_req.beta_angle < 0.0 || validator_req.beta_angle > 90.0 {
        return Json(ApiResponse::<Validator>::error("Beta angle must be between 0 and 90 degrees".to_string()));
    }

    if validator_req.efficiency < 0.0 || validator_req.efficiency > 1.0 {
        return Json(ApiResponse::<Validator>::error("Efficiency must be between 0 and 1".to_string()));
    }
...
async fn get_paginated_blocks(
    ...
    let start_block = if page > 1 { total_blocks.saturating_sub((page - 1) * limit) } else { total_blocks };
